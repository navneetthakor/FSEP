...
namespace WorkerService1.BL
{
    internal class EmailFormatter
    {
        public static EmailContent EmailInfoGetter(ServerModal sm, HealthCheckerModal hcm)
        {
            ...
            {
                To = cm.Client_email,
                //Subject = $"{sm.Server_name} is Down!!!",
                Subject = $"{serverName} is Down!!!",
                Body = $"Hello {cm.Client_name},<br><br> Your server {serverName} is returning following: <br> status code: {hcm.StatusCode} <br> response time: {hcm.ResponseTime} <br> Message: {hcm.ErrorMessage} <br> <br> Please check the server. <br> <br> Regards, <br> WebPulse Stack"
            };
        }
    }
}
